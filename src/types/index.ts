...
export interface SystemConfig {
  ...
  dataSource: 'SIMULATION' | 'REMOTE_WEBSOCKET' | 'PROMETHEUS';
  gatewayUrl: string;
...
// ==========================================
// Remote Backend Telemetry Contracts (WebSocket)
// ==========================================

export interface RemoteSensorAnalysis {
  status?: string;        // "warning", "normal", "critical", "high_risk", etc.
  risk_level?: string;    // "low", "medium", "high", "critical", etc.
  message?: string;       // LLM analysis insight / diagnostic message
  score?: number;         // optional composite score
  confidence?: number;
  trend?: 'STABLE' | 'IMPROVING' | 'DETERIORATING' | 'CRITICAL_RISK';
  model?: string;
  model_available?: boolean;
}

export interface RemoteSensorPayload {
  device_id: string;
  timestamp?: string | number;
  sensors: Record<string, number | null | undefined>;
  analysis?: RemoteSensorAnalysis;
}

export interface RemoteMultiDevicePayload {
  devices?: RemoteSensorPayload[];
}
...
export type TelemetryPacket = Record<string, any>;
