...
import { Brain, Cpu, AlertTriangle, ShieldCheck, TrendingUp, TrendingDown, ArrowUpRight, RefreshCw, Sparkles } from 'lucide-react';
...
export const AIInsightsPanel: React.FC = () => {
  const { riskAssessment, nodes, alerts, setActiveTab, config } = useApp();
  ...
  useEffect(() => {
    if (score > 0) playSoundEffect('gauge-fill');
  }, [score]);
  ...
  const triggerOllamaAssessment = useCallback(async () => {
    ...
    try {
      const telemetryPacket = {
        ...
        readings: nodes.length > 0 ? nodes.map(n => ({
          sensor_id: `${n.id}-DISP`,
          type: 'displacement',
          value: 6.4,
          unit: 'mm',
      ...
      setOllamaModel(result.model);
      setOllamaAvailable(result.model_available);
    } catch {
  ...
  return (
    <div className="cc-intel-section" style={{ flex: '0 0 auto' }}>
      ...
      <div className="cc-intel-body" style={{ padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {/* Ollama gemma3:4b Powered Analysis */}
        {riskAssessment.summaryRationale && (
          <div
            style={{
              background: 'rgba(38,101,253,0.08)',
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 9, fontWeight: 800, color: 'var(--color-primary-light)', textTransform: 'uppercase', letterSpacing: 0.5, display: 'flex', alignItems: 'center', gap: 4 }}>
                <Sparkles size={10} style={{ color: 'var(--color-primary)' }} />
                Ollama {ollamaModel}
                {!ollamaAvailable && <span style={{ color: 'var(--color-critical)', fontSize: 7 }}>OFFLINE</span>}
              </span>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{
                  ...
                  fontWeight: 700,
                  color: riskAssessment.riskLevel === 'CRITICAL' ? 'var(--color-critical)' : riskAssessment.riskLevel === 'WARNING' ? 'var(--color-warning)' : 'var(--color-normal)'
                }}>
                  {riskAssessment.riskLevel}
                </span>
                <button
                  type="button"
                  onClick={(e) => { e.stopPropagation(); triggerOllamaAssessment(); }}
                  disabled={isAssessing}
                  aria-label="Refresh Ollama analysis"
                  title="Refresh Ollama gemma3:4b analysis"
                  style={{
                    background: 'none',
                    border: '1px solid var(--border-subtle)',
                    borderRadius: 4,
                  ...
                  onMouseEnter={e => { if (!isAssessing) e.currentTarget.style.borderColor = 'var(--color-primary)'; }}
                  onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border-subtle)'}
                >
                  <RefreshCw size={10} style={{ color: 'var(--color-primary)', animation: isAssessing ? 'spin 1s linear infinite' : 'none' }} />
                </button>
              </div>
            </div>
            <div style={{ fontSize: 10.5, color: 'var(--text-bright)', lineHeight: 1.4 }}>
              {riskAssessment.summaryRationale}
            </div>
          </div>
        )}

        {/* Ollama Model Status Bar */}
        <div style={{
          display: 'flex',
          alignItems: 'center',
          gap: 6,
          ...
          <span style={{
            ...
            background: ollamaAvailable ? '#10b981' : '#ef4444',
            display: 'inline-block',
          }} />
          Ollama {ollamaModel} on {config.gatewayUrl || 'localhost:8000'}
        </div>
